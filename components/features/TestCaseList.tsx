...
export function TestCaseList({ currentKey, cases, onView, onEdit, onDelete, onShowCreate }: TestCaseListProps) {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <div className="flex items-start justify-between mb-2">
          <div>
            <div className="flex items-center gap-1.5 text-xs text-slate-400 mb-1">
              <svg width="11" height="11" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={1.5}><rect x="2" y="4" width="12" height="9" rx="1.5"/><path d="M5 4V3a3 3 0 0 1 6 0v1"/></svg>
              <span>{mod}</span>
              <span>/</span>
      ...
      {/* Filter bar */}
      <div ref={filterBarRef} className="flex items-center gap-2 px-6 py-2.5 border-b border-slate-200 bg-slate-50 relative">
        <div className="relative flex-1 max-w-[280px]">
          ...
          <input
            ...
            className="w-full pl-7 pr-3 py-1.5 border border-slate-200 rounded-lg text-xs font-sans bg-white text-slate-900 outline-none focus:border-blue-500"
          />
        </div>
        ...
        {activeFilterCount > 0 && (
          <button
            ...
            className="px-2.5 py-1.5 rounded-lg text-xs text-slate-500 hover:bg-slate-200 cursor-pointer"
          >
            Clear all
          </button>
        )}
      ...
      <div className="flex-1 overflow-auto">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center flex-1 py-24 gap-3 text-slate-400">
            <span className="text-4xl opacity-40">📋</span>
          ...
          <table className="w-full border-collapse text-xs">
            <thead>
              <tr>
                {['', 'ID', 'Title', 'Priority', 'Severity', 'Type', 'Feature', 'Updated', ''].map((h, i) => (
                  <th key={i} className="sticky top-0 z-10 bg-slate-50 px-3 py-2.5 text-left text-[11px] font-semibold text-slate-500 border-b border-slate-200 whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {filtered.map(tc => (
                ...
                >
                  <td className="px-3 py-2.5 w-9">
                    <input type="checkbox" onClick={e => e.stopPropagation()} className="accent-blue-600 w-3.5 h-3.5 cursor-pointer" />
                  </td>
                  <td className="px-3 py-2.5">
                    <span className="font-mono text-[11px] text-slate-400 font-semibold">{tc.id}</span>
                  </td>
                  <td className="px-3 py-2.5">
                  ...
                  <td className="px-3 py-2.5 text-slate-400 whitespace-nowrap">{tc.updated}</td>
                  <td className="px-3 py-2.5">
                    {hoveredRow === tc.id && (
                      <div className="flex items-center gap-1">
                        <button
                          onClick={e => { e.stopPropagation(); onEdit(tc.id); }}
                        ...
                        <button
                          ...
                          className="px-2 py-0.5 border border-slate-200 rounded text-[11px] text-slate-500 bg-white hover:bg-red-50 hover:text-red-600 hover:border-red-200 cursor-pointer"
                        >
                          Del
                        </button>
                      </div>
...
function FilterButton<T extends string>({ label, options, selected, onToggle, onClear, isOpen, onOpen }: FilterButtonProps<T>) {
  const count = selected.size;
  return (
    <div className="relative">
      <button
        onClick={onOpen}
        className={`flex items-center gap-1 px-3 py-1.5 border rounded-lg text-xs cursor-pointer transition-colors ${
          count > 0
      ...
      >
        {label}
        {count > 0 && (
          <span className="ml-1 px-1.5 py-px rounded-full bg-blue-600 text-white text-[10px] font-mono">{count}</span>
        )}
        <span className="text-[10px] ml-0.5">{isOpen ? '▴' : '▾'}</span>
      </button>

      {isOpen && (
        <div className="absolute top-[calc(100%+4px)] left-0 z-30 w-[180px] bg-white border border-slate-200 rounded-lg shadow-lg py-1.5">
          {options.map(opt => {
