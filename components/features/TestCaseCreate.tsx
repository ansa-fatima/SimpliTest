...
interface TestCaseCreateProps {
  ...
  onCancel: () => void;
  onSave: (data: {
    module: string; feature: string; priority: Priority; severity: Severity;
    type: TestType; title: string; desc: string; steps: string[]; expected: string;
  }) => void;
}

const TYPES: TestType[] = ['Functional', 'Regression', 'Smoke', 'Sanity', 'UI', 'API'];

export function TestCaseCreate({ modules, defaultModule, defaultFeature, onCancel, onSave }: TestCaseCreateProps) {
  ...
  const handleSave = () => {
    ...
    if (!stripHtml(stepsHtml)) { setError('Steps are required'); return; }
    onSave({
      module,
      feature,
      priority,
    ...
    <div className="flex flex-col flex-1 overflow-hidden bg-slate-50">
      ...
      <div className="flex items-center gap-2 px-4 py-2 bg-white border-b border-slate-200">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Button variant="primary" onClick={handleSave}>Save test case</Button>
        </div>
      </div>

      {/* Form */}
      <div className="flex-1 overflow-y-auto px-6 py-5">
        <div className="max-w-[860px] mx-auto bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-5 p-6">
          ...
          <div className="flex flex-wrap gap-4 items-end">
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                className="w-[150px] px-2.5 py-1.5 border border-slate-200 rounded-lg text-xs bg-white text-slate-900 outline-none focus:border-blue-500"
              >
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="text-[11px] text-slate-400 mt-1">
              Tip: use <span className="font-semibold">1. List</span> for numbered steps or <span className="font-semibold">• List</span> for bullets.
            </p>
          </div>
